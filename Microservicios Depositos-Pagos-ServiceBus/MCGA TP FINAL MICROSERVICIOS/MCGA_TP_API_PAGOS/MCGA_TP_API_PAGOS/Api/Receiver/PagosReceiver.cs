...
{
    public class PagosReceiver<T> : BackgroundService where T : class
    {
        ...
        public PagosReceiver(MessageManager messageManager, MessageManagerSettings messageManagerSettings, QueueSettings settings, ILogger<PagosReceiver<T>> logger, IServiceProvider serviceProvider)
        {
            ...
            queueName = settings?.Queues?.FirstOrDefault(q => q.Name == "Pagos").Type?.Name;

            if (queueName is null)
                queueName = settings?.QueuesString?.FirstOrDefault(q => q == "Pagos");
